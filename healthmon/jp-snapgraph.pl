...
# my @today = Today();
my $today_stamp = `/bin/rsh -l vplaces megan "/u/vplaces/scripts/healthmon/printToday.pl"`;
my $yest_stamp = `/bin/rsh -l vplaces megan "/u/vplaces/scripts/healthmon/printYesterday.pl"`;

# my $today_stamp = sprintf("%04d%02d%02d", @today);
# my $yest_stamp  = sprintf("%04d%02d%02d", Add_Delta_Days(@today, -1));

my $yfile       = $basedatadir . '/jp-snapshot.log.' . $yest_stamp;
my $tfile       = $basedatadir . '/jp-snapshot.log.' . $today_stamp;
