#
# getAllPlaces -- returns an array of places of any type
...
#
# NOTE: Make sure to change the first command in this script if 
#       perl5 is installed in a different directory.
...
sub getAllPlaces
{
  ...
  if ($mode eq "FILE") {
     ...
     if ($category == -1) {
       $fileName =  "AllPlaces.log";
     }
     else {
       $fileName =  "AllPlacesWithCateg.log";
     }
    ...
    if ($category != -1) {	# category specified
      $query = "exec vpplaces..getPlaces 2, $category, $index, $temp";
    }
    else {
      $query = "exec vpplaces..getPlaces 2, NULL, $index, $temp";
    }
