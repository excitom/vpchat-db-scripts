#
# getCoolPlaces -- returns an array of "Cool" Places
#
...
sub getCoolPlaces
{
  ...
  if ($mode eq "FILE") {
     ...
     if ($category == -1) {
       $fileName =  "CoolPlaces.log";
     }
     else {
       $fileName =  "CoolPlacesWithCateg.log";
     }
    ...
    if ($category != -1) {	# category specified
      $query = "exec vpplaces..getPlaces 0, $category, $index, $temp";
    }
    else {
      $query = "exec vpplaces..getPlaces 0, NULL, $index, $temp";
    }
