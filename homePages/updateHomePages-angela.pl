...
###print "$dir\n";
			...
			###print "/usr/local/bin/access -u $dir\n";
			`mv -- $dir $saveDir`;
			###print "mv -- $dir $saveDir\n";
			`rm -f -- "$docRoot/$URL2"` if ($URL ne $URL2);
			###print "rm -f -- \"$docRoot/$URL2\"\n";
			my $url = "$G_config{'membersURL'}/$strVal{$userID}";
###print "$url\n";
		...
		#
			...
			if (! -d "$docRoot/$URL") {
				###print "mkdir $docRoot/$URL; chmod 777 $docRoot/$URL\n";
				`mkdir $docRoot/$URL; chmod 777 $docRoot/$URL`;
			}
			my $dir = "$saveDir/$URL";
###print "$dir\n";
			`mv -- $dir $docRoot`;
			###print "mv -- $dir $docRoot\n";
			`ln -s -- $docRoot/$URL $docRoot/$URL2` if ($URL ne $URL2);
			###print "ln -s -- $docRoot/$URL $docRoot/$URL2\n";
			`/usr/local/bin/access -u $docRoot/$URL`;
			###print "/usr/local/bin/access -u $docRoot/$URL\n";
