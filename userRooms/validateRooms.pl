#!/usr/bin/perl
#
# Periodically check the user-created room list and report
# anomalies.
...
#####################################################
#
# Subroutine: check for expired accounts with listed rooms
#
sub checkExpiredAccounts {

  print LOG "----------\nCheck for EXPIRED accounts with listed rooms\n";
  print "----------\nCheck for EXPIRED accounts with listed rooms\n";
  my $found = 0;
...
#####################################################
#
# Subroutine: check for suspended accounts with listed rooms
#
sub checkSuspendedAccounts {

  print LOG "----------\nCheck for SUSPENDED accounts with listed rooms\n";
  print "----------\nCheck for SUSPENDED accounts with listed rooms\n";
  my $found = 0;
...
FROM vpusers..userAccounts a,
     vpplaces..userPlaces u
WHERE a.accountID = u.accountID
AND   u.URL NOT IN (SELECT URL FROM vpplaces..persistentPlaces)
SQL
  my $sth = $G_dbh->prepare($sql);
  die 'Prepare failed' unless (defined($sth));
  $sth->execute;
...
####
    ...
    $sql = "vpplaces..delUserPlace $accountID, \"$URL\"";
    $sth = $G_dbh->prepare($sql);
    $sth->execute;
    do {
      while (@row = $sth->fetchrow() ) {
...
print LOG "$now\tChecking user created rooms for $G_config{'communityName'}\n";
print "$now\tChecking user created rooms for $G_config{'communityName'}\n";
